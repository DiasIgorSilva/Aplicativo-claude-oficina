...
// ── Gerador de PDF (Focado em Data de Entrega) ──
...
<body>
  <div class="hdr">
    ...
    <div style="text-align:right"><strong>Período:</strong> ${fmtDate(dateFrom)} até ${fmtDate(dateTo)}</div>
  </div>
  <div class="resumo">
    <div class="card-res">Peças: <br/><strong>${fmt(tP)}</strong></div>
    <div class="card-res">Mão de Obra: <br/><strong>${fmt(tL)}</strong></div>
    <div class="card-res" style="border-color:#f97316">Total Geral: <br/><strong>${fmt(tP+tL)}</strong></div>
  </div>
  <table>
    <thead><tr><th>Entrega</th><th>Veículo</th><th>Descrição</th><th>M.O.</th><th>Peças</th><th>Total</th></tr></thead>
    <tbody>
      ${fS.map(s => `
        <tr>
    ...
    <Section title="Base Master" action={<button className="btn-primary" onClick={()=>open()}>+ Novo</button>}>
      ...
      <div className="card" style={{padding:0, overflow:"hidden"}}>
        <div className="table-wrap">
          <div className="table-header" style={{gridTemplateColumns:"1.5fr 1.5fr 1fr 80px"}}><span>Veículo</span><span>Cliente</span><span>Telefone</span><span></span></div>
          {filtered.map(v=>(
            <div key={v.id} style={{display:"grid", gridTemplateColumns:"1.5fr 1.5fr 1fr 80px", alignItems:"center", padding:14, borderBottom:"1px solid #1e2736", minWidth:"600px"}}>
              <div>
      ...
      {modal && <div className="modal-bg" onClick={close}><div className="modal" onClick={e=>e.stopPropagation()}><h3>Cadastro de Carro</h3><Field label="Placa" value={form.plate} onChange={v=>setForm({...form,plate:v.toUpperCase()})}/><Field label="Marca" value={form.brand} onChange={v=>setForm({...form,brand:v})}/><Field label="Modelo" value={form.model} onChange={v=>setForm({...form,model:v})}/><div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}><Field label="Ano" value={form.year} onChange={v=>setForm({...form,year:v})}/><Field label="Cor" value={form.color} onChange={v=>setForm({...form,color:v})}/></div><Field label="Cliente" value={form.owner} onChange={v=>setForm({...form,owner:v})}/><Field label="Telefone" value={form.phone} onChange={v=>setForm({...form,phone:v})}/><button className="btn-primary" style={{width:"100%", marginTop:10}} onClick={save}>Salvar</button></div></div>}
...
function Services({services,setServices,vehicles,mapS}) {
  ...
  return (
    <Section title="Fluxo Oficina" action={<button className="btn-primary" onClick={()=>open()}>+ Entrada</button>}>
      <div className="card" style={{padding:0, overflow:"hidden"}}>
        <div className="table-wrap">
          <div className="table-header" style={{gridTemplateColumns:"2fr 1.2fr 0.8fr 1fr 60px"}}><span>Serviço</span><span>Veículo</span><span>M.O.</span><span>Status</span><span></span></div>
          {services.map(s=>(
            <div key={s.id} style={{display:"grid", gridTemplateColumns:"2fr 1.2fr 0.8fr 1fr 60px", alignItems:"center", padding:14, borderBottom:"1px solid #1e2736", minWidth:"650px"}}>
