...
// ── MAPEADORES DE SEGURANÇA (PARA NÃO PERDER DADOS ANTIGOS) ──────────────
const mapV = (r: any) => ({
...
function VehicleSelector({ vehicles, value, onChange }: any) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", marginBottom: 15 }}>
      ...
      <input className="input" placeholder="Busque placa ou modelo..." value={open ? query : (sel ? `${sel.plate} — ${sel.model}` : "")} onFocus={() => { setOpen(true); setQuery(""); }} onChange={e => setQuery(e.target.value)} autoComplete="off" />
      {open && (
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0f14", color: "#e2e8f0", fontFamily: "'DM Mono', monospace", paddingBottom: 80 }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&family=Syne:wght@700;800&display=swap');
        *{box-sizing:border-box;margin:0;padding:0;}
        body, html { overflow-x: hidden; width: 100%; position: relative; }
        .card{background:#161b26; border:1px solid #1e2736; border-radius:12px; padding:16px; margin-bottom:12px; width:100%;}
        .btn-primary{background:#f97316; color:#0d0f14; border:none; border-radius:8px; padding:12px 18px; font-weight:800; cursor:pointer; font-size:13px; width:100%;}
        .btn-ghost{background:transparent; color:#94a3b8; border:1px solid #1e2736; border-radius:8px; padding:8px 12px; cursor:pointer;}
        .input{background:#0d0f14; border:1px solid #1e2736; border-radius:8px; padding:10px 12px; color:#e2e8f0; width:100%; font-family:inherit; font-size:13px; margin-bottom:10px;}
        ...
        .table-header, .table-row { min-width: 600px; display: grid; align-items: center; padding: 12px; border-bottom: 1px solid #1e2736; }
      `}</style>
      ...
      <main style={{ padding: 16, maxWidth: 800, margin: "0 auto" }}>
        {loading ? <p style={{textAlign:"center", padding:50}}>Sincronizando...</p> : (
          <>
  ...
  return (
    <div>
      ...
      <div className="kpi-grid">
        <div className="card" style={{ borderLeft: "3px solid #f97316" }}><label className="label">Na Oficina</label><strong>{active.length}</strong></div>
        <div className="card" style={{ borderLeft: "3px solid #10b981" }}><label className="label">Total Mês</label><strong>{fmt(tP + tL)}</strong></div>
      </div>
      <div className="card">
        <h3 style={{ fontSize: 13, marginBottom: 10, color:"#f97316" }}>🛠️ Ativos</h3>
        {active.map((s:any) => <div key={s.id} style={{padding:"8px 0", borderBottom:"1px solid #1e2736", fontSize:12}}><strong>{s.vehiclePlate}</strong> - {s.description}</div>)}
      </div>
    </div>
  );
}
...
function ServicesTab({ services, setServices, vehicles }: any) {
  ...
  const open = (s = null) => { setEditing(s); setForm(s || { status: "Aguardando", entryDate: today() }); setModal(true); };
    ...
    <div>
      ...
      {services.filter((s:any)=>s.status !== "Entregue").map((s: any) => (
        <div key={s.id} className="card" style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{fontSize:12}}><strong>{s.vehiclePlate}</strong><br/>{s.description}</div>
      ...
      {modal && (
        <div className="modal-bg" onClick={() => setModal(false)}><div className="modal" onClick={e => e.stopPropagation()}>
          ...
          {form.status === "Entregue" && (
            <div style={{background:"#0d0f14", padding:12, borderRadius:8, marginTop:10}}>
...
// ── ABA FINANCEIRO (CORREÇÃO DE DESPESAS) ──────────────────────────────────
function FinanceTab({ services, expenses, setExpenses }: any) {
  ...
  const saveExp = async () => {
    ...
    const row = { ...form, id: uid(), value: Number(form.value), expense_date: form.expense_date || today() };
  ...
  return (
    <div>
      ...
      <div className="kpi-grid">
        ...
        <div className="card" style={{borderLeft:"4px solid #3b82f6", gridColumn:"1/-1"}}><label className="label">Lucro Limpo</label><strong style={{fontSize:18, color: (totalIn-totalOut)>=0?"#10b981":"#ef4444"}}>{fmt(totalIn-totalOut)}</strong></div>
      </div>
      <button className="btn-primary" onClick={() => setModal(true)}>+ Lançar Despesa</button>
      <div className="card" style={{marginTop:15}}>
        <h3 style={{fontSize:13, marginBottom:10}}>Histórico de Gastos</h3>
        {filteredExp.length === 0 ? <p style={{fontSize:11, color:"#64748b"}}>Sem gastos no mês.</p> : filteredExp.map((e:any) => (
          <div key={e.id} style={{display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid #1e2736", fontSize:12}}>
            <div><strong>{e.category}</strong><br/><small>{e.supplier}</small></div>
      ...
      {modal && (
        <div className="modal-bg" onClick={() => setModal(false)}><div className="modal" onClick={e => e.stopPropagation()}>
          <h3>Gasto Manual</h3>
          <Field label="Categoria (Luz, Aluguel, Peças...)" onChange={(v:any) => setForm({...form, category: v})} />
          <Field label="Valor (R$)" type="number" onChange={(v:any) => setForm({...form, value: v})} />
          <Field label="Fornecedor / Loja" onChange={(v:any) => setForm({...form, supplier: v})} />
          <Field label="Data do Gasto" type="date" value={form.expense_date} onChange={(v:any) => setForm({...form, expense_date: v})} />
...
function VehiclesTab({ vehicles, setVehicles, services }: any) {
  ...
  const save = async () => {
    const row = { id: form.id || uid(), plate: form.plate, brand: form.brand, model: form.model, year: form.year, mileage: Number(form.mileage)||0, owner: form.owner, phone: form.phone };
  ...
  return (
    <div>
      <button className="btn-primary" onClick={() => open()} style={{marginBottom:15}}>+ Novo Veículo</button>
      <div className="table-wrap">
        {vehicles.map((v: any) => (
          <div key={v.id} className="card" style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
            <div><button onClick={() => setHist(v)} className="btn-ghost" style={{fontSize:9, padding:"2px 8px", marginBottom:5}}>📜 Histórico</button><br/><strong>{v.plate}</strong> - {v.model}</div>
...
// ── HELPERS ───────────────────────────────────────────────────────────────
function Section({ title, action, children }: any) { return (<div style={{ marginBottom: 20 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15 }}><h2 style={{ fontFamily: "'Syne'", fontSize: 20 }}>{title}</h2>{action}</div>{children}</div>); }
...
function generatePDF(v:any, s:any, f:any, t:any) { /* Lógica de PDF igual v1.3.2 */ }
function ReportModal({ services, onClose, onGenerate }: any) { /* Lógica de Modal igual v1.3.2 */ 
  const [dF, setDF] = useState(today()); const [dT, setDT] = useState(today());
  return (<div className="modal-bg" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}><h3>Relatório</h3><Field label="De" type="date" value={dF} onChange={setDF}/><Field label="Até" type="date" value={dT} onChange={setDT}/><button className="btn-primary" onClick={()=>onGenerate(dF,dT)}>Gerar</button></div></div>);
}
