...
// Lista de marcas atuantes no Brasil desde 1990
const BR_BRANDS = [
  ...
  return (
    <Section title="Base Master" action={<button className="btn-primary" onClick={()=>open()}>+ Novo</button>}>
      ...
      {modal && <div className="modal-bg" onClick={close}><div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}><Field label="Placa" value={form.plate} onChange={v=>setForm({...form,plate:v.toUpperCase()})}/>
        <SelectField label="Marca" value={form.brand} onChange={v=>setForm({...form,brand:v})} options={BR_BRANDS} placeholder="Selecione"/></div>
        <Field label="Modelo" value={form.model} onChange={v=>setForm({...form,model:v})}/>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}><Field label="Ano" value={form.year} onChange={v=>setForm({...form,year:v})}/><Field label="Cor" value={form.color} onChange={v=>setForm({...form,color:v})}/></div>
