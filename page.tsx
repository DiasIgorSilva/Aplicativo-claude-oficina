    ...
    function VehiclesTab({ vehicles, services, loadAll, onZoomPhoto }) {
      ...
      const save = async () => {
        ...
        const row = { 
          ...
          year: form.year, engine: form.engine || "", color: form.color, owner: form.owner, phone: form.phone, notes: form.notes, mileage: Number(form.mileage) || 0 
        };
      ...
      const filtered = vehicles.filter(v => !search || (v.plate||"").toLowerCase().includes(search.toLowerCase()) || (v.owner||"").toLowerCase().includes(search.toLowerCase()) || (v.model||"").toLowerCase().includes(search.toLowerCase()));

      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <input className="input" style={{ maxWidth: 300 }} placeholder="🔍 Buscar por placa, dono ou modelo..." value={search} onChange={e => setSearch(e.target.value)} />
            <button className="btn-primary" onClick={() => open()}>+ Novo Veículo</button>
          </div>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#141824", color: "#94a3b8", textAlign: "left" }}>
                    <th style={{ padding: "12px 16px" }}>Veículo</th>
                    <th style={{ padding: "12px 16px" }}>Placa</th>
                    <th style={{ padding: "12px 16px" }}>Cliente / Contato</th>
                    <th style={{ padding: "12px 16px" }}>KM Registrado</th>
                    <th style={{ padding: "12px 16px", textAlign: "right" }}>Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={5} style={{ padding: 30, textAlign: "center", color: "#64748b" }}>Nenhum veículo encontrado.</td></tr>
                  ) : (
                    filtered.map(v => (
                      <tr key={v.id} style={{ borderBottom: "1px solid #1c2234" }}>
                        <td style={{ padding: "12px 16px" }}>
                          <strong style={{ color: "#f8fafc" }}>{v.brand} {v.model}</strong>
                          {v.engine ? <span style={{ fontSize: 11, color: "#94a3b8", marginLeft: 6 }}>({v.engine})</span> : null}
                          <div style={{ fontSize: 11, color: "#64748b" }}>Ano: {v.year || "—"}</div>
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          <span className="plate-badge">{v.plate}</span>
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          <div style={{ color: "#f8fafc", fontWeight: 600 }}>{v.owner || "—"}</div>
                        ...
                        <td style={{ padding: "12px 16px", textAlign: "right" }}>
                          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
                            <button className="btn-ghost" style={{ padding: "6px 10px", fontSize: 12 }} onClick={() => { setSelectedV(v); setHistoryModal(true); }}>📜 Histórico</button>
                            <button className="btn-ghost" style={{ padding: "6px 10px", fontSize: 12 }} onClick={() => open(v)}>✏️ Editar</button>
                          </div>
                        </td>
                      </tr>
                    ))
          ...
          {modal && (
            <div className="modal-bg" onClick={close}>
              <div className="modal" onClick={e => e.stopPropagation()}>
                <h3>{editing ? "Editar Veículo" : "Novo Cadastro Master"}</h3>
          ...
          {historyModal && (
            <div className="modal-bg" onClick={() => setHistoryModal(false)}>
              <div className="modal" onClick={e => e.stopPropagation()}>
                <h3>📜 Histórico do Veículo: {selectedV?.plate}</h3>
                <div style={{ maxHeight: 350, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginTop: 10 }}>
                  {services.filter(s => (s.vehiclePlate || "").toUpperCase() === (selectedV?.plate || "").toUpperCase()).length === 0 ? (
                    <p style={{ color: "#64748b", textAlign: "center", padding: 20 }}>Nenhum serviço registrado para este veículo.</p>
                  ) : (
                    services.filter(s => (s.vehiclePlate || "").toUpperCase() === (selectedV?.plate || "").toUpperCase()).map(s => (
                      <div key={s.id} style={{ background: "#0b0d14", border: "1px solid #1c2234", padding: 12, borderRadius: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                          <span style={{ fontSize: 11, color: "#64748b" }}>Data: {fmtDate(s.entryDate)}</span>
                          <StatusBadge status={s.status} />
                        </div>
                        <div style={{ fontSize: 13, color: "#f8fafc", fontWeight: 600 }}>{s.description}</div>
                        <div style={{ fontSize: 12, color: "#10b981", marginTop: 6, fontWeight: 700 }}>Total: {fmt((Number(s.partsValue)||0) + (Number(s.laborValue)||0))}</div>
                      </div>
                    ))
                  )}
                </div>
