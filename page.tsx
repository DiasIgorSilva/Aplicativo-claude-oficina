...
const { useState, useEffect, useRef, useMemo, useCallback } = React;
  ...
  const [driveUrl, setDriveUrl] = useState(DEFAULT_DRIVE_URL);
      ...
      async function loadAll(isInitial = false) {
        ...
        const vRes = await supabaseClient.from("vehicles").select("*").or("is_test.is.null,is_test.eq.false").order("created_at", { ascending: false });

        if (vRes.data) setVehicles(vRes.data.map(mapV));



        const sRes = await supabaseClient.from("services").select("*").or("is_test.is.null,is_test.eq.false").order("created_at", { ascending: false });
