...
const { useState, useEffect, useRef, useMemo, useCallback } = React;
  ...
  const [driveUrl, setDriveUrl] = useState(DEFAULT_DRIVE_URL);
    ...
    .workshop-car-card { cursor: pointer; transition: all 0.2s ease; }
          ...
          {/* BANNER SANDBOX DE TESTES ISOLADO */}
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <span>
                <strong>AMBIENTE DE TESTES (SANDBOX ISOLADO):</strong> Suas alterações aqui são salvas localmente e <strong>NÃO afetam o banco de dados oficial de produção</strong>.
              </span>
            </div>
            <button
              onClick={async () => {
                if (confirm("Deseja recarregar uma cópia limpa dos dados reais da produção para este ambiente de teste? Suas alterações locais de teste serão substituídas pela cópia atual do banco.")) {
                  setLoading(true);
                  await syncFromProduction();
                  await loadAll(false);
                  setLoading(false);
